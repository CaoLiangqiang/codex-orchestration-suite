...
<!-- suite-integration:start -->

## Project Sessions Integration

Keep Team Mode independently invocable. Apply this integration only when an enclosing `orchestrate-project-sessions` controller or durable worker is present; standalone Team Mode behavior above remains unchanged.

Before spawning inside a durable worker, read [references/project-sessions-integration.md](references/project-sessions-integration.md) and [references/coordination-contract.md](references/coordination-contract.md). Obey the controller's explicit `Subagent policy`, count children against its shared concurrency budget, and report child sessions and evidence back to the durable task. If the work instead needs durable history, independent user steering, or a separate writable checkout, return `Durable task required:` with the reason; Team Mode must not create that task itself.
