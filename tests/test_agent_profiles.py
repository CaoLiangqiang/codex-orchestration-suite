...
class AgentProfileTests(unittest.TestCase):
    def test_bundled_profile_copies_match_root_templates(self) -> None:
        bundled = ROOT / "skills" / "team-mode" / "assets" / "agent-profiles"
        for filename in PROFILES:
            with self.subTest(filename=filename):
                self.assertEqual(
                    (ROOT / "agents" / filename).read_bytes(),
                    (bundled / filename).read_bytes(),
                )
